    ...
    <div id="myOrderItem">
        ...
        <div class="itemCenter">
            <div class="centerLeft">
                <img v-for="(cartGoods, index) in order.cart_shop.slice(0, 5)" :key="index" :src="cartGoods.small_image" alt="" width="30">
            </div>
            <div class="centerRight">{{order.cart_shop.length > 6 ? '…' : ''}}</div>
        </div>
        <div class="itemBottom">
            <van-cell title=" " :value="`共${order.cart_shop.length}份商品 实付¥${(order.shop_price + order.dis_price)+0.01}`"></van-cell>
        </div>
    </div>
</template>
